...
const OEMsTable = () => {
    const { loading, data, selectedOem } = useSelector((store: RootState) => store.oem);
            ...
            {loading ? <h1>Loading...</h1> :
                <div className={styles['OEM-specs']}>
                    <table>
                        ...
                        <tbody>
                            {
                                data.map((el: OEMsSpecProps, indx: number) => <tr key={el._id}>
                                    <td>{indx + 1}</td>
                                    <td>{el?.brand}</td>
                                    <td>{el?.model}</td>
                                    <td>{el?.year}</td>
                                    ...
                                    <td>
                                        <button
                                            style={{
                                                backgroundColor: `${el._id === selectedOem ? "green" : "rgba(0,0,0,0.1)"}`,
                                                color: `${el._id === selectedOem ? "#fff" : "grey"}`
                                            }}
